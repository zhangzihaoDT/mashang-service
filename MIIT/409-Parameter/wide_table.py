#!/usr/bin/env python3
"""
Parse 50 new car models from batch 409 scan and generate a wide table
by merging 附件1 (brand .md detail pages) and 附件2 (车船税 JSON).

Outputs: MIIT/batch_409_wide_table.csv + MIIT/batch_409_wide_table.md
...
def read_brand_md(brand: str, model_id: str, batch: str = DEFAULT_BATCH) -> dict | None:
    ...
    for f in brand_dir.iterdir():
        if f.suffix == ".md" and f.stem.startswith(model_id):
            return _parse_md_file(f)
    return None
