...
BATCH = "第409批"
BATCH_DIR_PREFIX = "409"          # 归档目录前缀，如 409-小米 / 410-零跑
ARCHIVE_DATE = "2026-07-13"       # 报告页脚存档时间

# 批次 → (公示日期, 对应车船税批次)
BATCH_META = {
    "409": {"archive_date": "2026-07-13", "tax_batch": 88},
    "410": {"archive_date": "2026-08-07", "tax_batch": 89},
}
...
def generate_brand_section_html(brand_name: str, brand_dir: Path, tax_index: dict, name_map: dict | None = None) -> str:
    ...
    groups = gmr.group_models(model_infos, tax_index, name_map)
...
def generate_category_html(category: str, tax_path: Path, output_dir: Path, dry_run: bool = False) -> str:
    ...
    for brand_name in brands:
        brand_dir = here / f"{BATCH_DIR_PREFIX}-{brand_name}"
        if not brand_dir.exists():
            continue
        models = load_brand_models(brand_dir, tax_index)
        if not models:
            continue
        groups = gmr.group_models(models, tax_index, name_map)
        total_models += len(models)
        total_groups += len(groups)
        active_brands += 1

        nav_links += f"""<a href="#{brand_name}">{brand_name}</a>"""
        sections += generate_brand_section_html(brand_name, brand_dir, tax_index, name_map)
...
def _apply_batch(batch: str) -> Path:
    ...
    if meta:
        ARCHIVE_DATE = meta["archive_date"]
        return HERE / f"车型清单_第{meta['tax_batch']}批车船税.json"
    return TAX_JSON_DEFAULT
