#!/usr/bin/env python3
"""
汽车市场情报监控脚本 — Auto Launch Monitor (v0.2)

查询指定日期范围内中国市场的汽车新车上市、预售、发布会、首发亮相、开启交付等产品投放事件，输出可追溯的结构化市场情报报告。
...
体系位置:
  ...
  ├── 新车事件监测 auto_launch_monitor.py     <- 当前 v0.2
...
EVENT_TYPES = ["发布会", "上市", "预售", "首发亮相", "开启交付", "官图发布", "媒体预热", "其他"]
...
MAINSTREAM_DOMAINS = [
    ...
    "bitauto.com", "gasgoo.com", "news18a.com",
    "xchuxing.com", "sina.com.cn", "sohu.com",
...
def parse_args():
    parser = argparse.ArgumentParser(description="汽车市场情报监控")
...
def build_search_queries(start, end, market, topic, max_results):
    topics = ["新车发布会", "新车上市", "新车预售", "新车亮相", "开启交付", "新车发布"]
    date_range = f"{start} 到 {end}"
    return [(f"{date_range} {market} {t}", t) for t in topics]
...
def extract_events_from_markdown(markdown_text, url, source_title, start_date=None, end_date=None):
    events = []
    text = markdown_text
    event_keywords = {
        "发布会": ["发布会", "正式发布"],
...
def aggregate_events(events):
    ...
    for e in events:
        ...
        b["source_types"].add(e["source_type"])
        if len(e["evidence"]) > len(b["evidence"]):
            b["evidence"] = e["evidence"]
        b["confidences"].append(e.get("confidence", "中"))

    aggregated = []
    for key, b in buckets.items():
        has_official = "official" in b["source_types"]
        mainstream_count = sum(1 for st in b["source_types"] if st == "mainstream")
        ...
        aggregated.append({
            ...
            "source_type": "|".join(sorted(b["source_types"])),
...
def main():
    ...
    queries = build_search_queries(start_str, end_str, args.market, args.topic, args.max_results)
    ...
    high_priority = [r for r in deduped if classify_domain(r.get("url", "")) == "official" or classify_domain(r.get("url", "")) == "mainstream"]
    normal_priority = [r for r in deduped if r not in high_priority]
    high_priority.sort(key=lambda x: x.get("score", 0), reverse=True)
    normal_priority.sort(key=lambda x: x.get("score", 0), reverse=True)

    candidates = (high_priority + normal_priority)[:args.max_results]
    ...
    for page in scraped_pages:
        ...
        events = extract_events_from_markdown(page["markdown"], page["url"], source_title,
    ...
    if args.format == "json":
        output = {"summary": summary, "events": events}
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(format_markdown(events, summary, start_str, end_str, args.topic))
        print(format_markdown(events, summary, start_str, end_str, args.topic))
...
def format_markdown(events, summary, start_str, end_str, topic):
    ...
    lines.append("| **低** | 自媒体、论坛、二次转载、无明确日期 |")
    lines.append("")
    ...
    lines.append("| **待确认** | 媒体预热、推测、预告，无官方确认 |")
    lines.append("")
