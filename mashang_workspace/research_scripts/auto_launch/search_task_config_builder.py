...
def _load_brand_watchlist():
    path = WORKSPACE_ROOT / "promptbuilders" / "auto_launch" / "configs" / "priority_brand_watchlist.yaml"
    if not path.exists():
        return {}
    with open(path) as f:
...
def _load_model_watchlist():
    path = WORKSPACE_ROOT / "promptbuilders" / "auto_launch" / "configs" / "ls8_competitor_watchlist.yaml"
    if not path.exists():
        return {}
    with open(path) as f:
        return yaml.safe_load(f)
