...
def _load_watchlists():
    brand_path = WORKSPACE_ROOT / "promptbuilders" / "auto_launch" / "configs" / "priority_brand_watchlist.yaml"
    model_path = WORKSPACE_ROOT / "promptbuilders" / "auto_launch" / "configs" / "ls8_competitor_watchlist.yaml"
    brands = {}
    models = {}
...
def _load_event_types():
    path = WORKSPACE_ROOT / "promptbuilders" / "auto_launch" / "configs" / "event_types.yaml"
    if not path.exists():
        return {}
    with open(path) as f:
...
def _load_source_tiers():
    path = WORKSPACE_ROOT / "promptbuilders" / "auto_launch" / "configs" / "source_tiers.yaml"
    if not path.exists():
        return {}
    with open(path) as f:
        return yaml.safe_load(f)
