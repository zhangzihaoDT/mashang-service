...
# -*- coding: utf-8 -*-
"""
上市以来零售锁单的配置分布报告（默认代际 DM2；--gen 可指定任意 series_group_logic 代际）

数据源：
  - dataset/order_data.parquet（锁单：<gen> series_group，上市日 time_periods.<gen>.end 起）
  - dataset/config_attribute.parquet（选配：order_config_to_parquet.py 增量更新后含最新代际）
...
用法：
  python research_scripts/lock_config_distribution.py                       # 默认 DM2
  python research_scripts/lock_config_distribution.py --gen CM3
  python research_scripts/lock_config_distribution.py --include-test-drive
  python research_scripts/lock_config_distribution.py --gen CM3 --format json --output outputs/tables/
"""
...
def load_order(gen: str = DEFAULT_GEN) -> tuple[pd.DataFrame, pd.Timestamp]:
    bd = load_business_definition(_BUSINESS_DEF)
    periods = bd.get("time_periods", {})
    if gen not in periods:
        raise ValueError(f"代际 {gen!r} 未在 business_definition.time_periods 中定义")
    end = pd.Timestamp(periods[gen]["end"]).normalize()
...
def compute_lock_config_distribution(
    ...
    """<gen> 上市以来锁单配置分布（可复用；供 CLI 与 launch 报告模块 7 引用）。

    gen = series_group_logic 代际标签（默认 DM2；可为任意 business_definition.time_periods
    中的代际，如 CM1/CM2/CM3）。计算层通用，不含车型特有配置语义
    （后者见 ls8_configuration_selection_report.py）。

    返回结构：
      - gen / n / covered / core_complete / core_total_orders
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="上市以来零售锁单配置分布（默认 DM2；--gen 指定代际）")
    p.add_argument("--gen", type=str, default=DEFAULT_GEN,
                   help=f"代际（series_group_logic 标签，默认 {DEFAULT_GEN}），如 DM2 / CM1 / CM2 / CM3")
    ...
    contract = build_success_contract(
        script="research_scripts/lock_config_distribution.py",
        command="python research_scripts/lock_config_distribution.py" +
                (f" --gen {gen}" if gen != DEFAULT_GEN else "") +
