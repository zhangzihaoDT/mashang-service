...
def render_daily_monitor_summary(payload: dict) -> str:
    ...
    wp = payload.get("window_policy", {})
    cew = wp.get("confirmed_event_window", {})
    dsw = wp.get("discovery_signal_window", {})
    cw = wp.get("context_window", {})
    lines.append(f"- confirmed_event_window: {cew.get('primary_start','unknown')} ~ {cew.get('end','unknown')}\n")
    lines.append(f"- discovery_signal_window: {dsw.get('default_start','unknown')} ~ {dsw.get('end','unknown')}\n")
    lines.append(f"- context_window: {cw.get('start','unknown')} ~ {cw.get('end','unknown')}\n")
