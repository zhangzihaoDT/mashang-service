...
| 角色 | 品牌 | 车型 | 说明 |
|------|------|------|------|
| **事件车型（event_model）** | 小米 | YU7 | 本次发生上市/预售/发布会等事件的车型 |
| **本品车型（our_model）** | （未指定） | （未指定） | 我方被影响车型 |
| **竞争战场** | — |  | 同战场竞品背景 |

本报告的核心问题是：**event_model（小米 YU7）本次事件，对 our_model（（未指定） （未指定））造成什么影响？**
...
| 目标车型 | YU7 |
| 目标车型竞争分组（group_id） |  |
| target_group 来源 | unknown |
| target_group 已解析 | 否 |
| battle_field_id 字段可用 |  |
| ecosystem_group（品牌阵营）可用 |  |
| group 字段可用（legacy） | 否 |
| 竞品匹配主字段 |  |
| group taxonomy 归一化 |  |
| 同战场竞品数量 | 0 |
...
| 本品品牌 | （未指定） |
| 本品车型 | （未指定） |
| 关注竞品（战场背景） | 特斯拉Model Y,小鹏G6,理想L6,问界M5,智界R7 |
...
**目标**：分析 **小米 YU7** 本次事件的市场影响，收集价格、权益、配置、舆论、竞品对标等信息，评估其市场定位和竞争力。
