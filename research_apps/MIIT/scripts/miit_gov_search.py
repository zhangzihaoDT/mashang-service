...
从 config/brand_watchlist.yaml 读取品牌列表，
在指定批次的 MIIT 新产品公示中搜索各品牌的新车申报信息。

用法:
  python scripts/miit_gov_search.py                    # 搜索第 409 批所有品牌
  python scripts/miit_gov_search.py --batch 409        # 指定批次
...
from miit_paths import WATCHLIST_PATH, DEFAULT_BATCH, get_batch_config  # noqa: E402

# ── MIIT API 配置 ──────────────────────────────────────────────────
MIIT_API = "https://www.miit.gov.cn/api-gateway/jpaas-publish-server/front/page/build/unit"
# 批次 page_id / index 统一维护在 config/batches.yaml，见 miit_paths.get_batch_config
...
def search_batch(batch: str, cpsb: str = "", qymc: str = "") -> dict:
    ...
    base_params = {**BASE_PARAMS_TEMPLATE, "pageId": cfg["page_id"]}
