...
def test_run_day_dry_run():
    ...
    assert "outputs" in r
    for k in ("run_dir", "manifest", "audit", "source_audit_json", "source_audit_md", "summary"):
...
def test_run_day_report_file():
    r = run_day(monitor_date="2026-07-09", live=False)
    # report is generated by run_brand_daily_report
    report_path = r["outputs"].get("report_manifest", "")
    if report_path:
        report = Path(report_path).read_text(encoding="utf-8")
        assert len(report) > 50
    print(f"[PASS] test_run_day_report_file")
...
def test_run_day_report_manifest():
    r = run_day(monitor_date="2026-07-09", live=False)
    assert "report_manifest" in r["outputs"]
    print(f"[PASS] test_run_day_report_manifest")
